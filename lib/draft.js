...
// After a suspend, Hyprland can keep rendering PQ while the panel has dropped
// out of HDR mode (everything looks washed out): it only re-sends the HDR
// metadata when its own idea of the output changes. Two rules that differ
// only in the preset make it do exactly that: sRGB first, then the request
// as it was. Bit depth stays, so neither step is a modeset. Null when the
// monitor isn't asking for HDR (or an ICC profile replaces the preset).
function hdrResend(cfg) {
  if (!cfg.enabled || !M.isHdr(cfg.cm) || cfg.icc) return null;
  return { first: toRule(merge(cfg, { cm: "srgb" })), second: toRule(cfg) };
}
...
function verify(requested, monitors) {
  ...
  requested.forEach(function (want) {
    ...
    if (want.cm !== "auto" && !want.icc && want.cm !== got.cm) {
      issues.push(M.isHdr(want.cm) && !M.isHdr(got.cm) && !hdrForced(want)
        ? want.name + " fell back to " + M.colorPresetLabel(got.cm) + ": Hyprland doesn't think it supports HDR"
          + (want.supports_hdr === 1 ? " (HDR also needs wide color support; set Wide color to On)." : ". Try “Force HDR”.")
...
// Which HDR and 10-bit controls the color panel offers, given what the EDID
// supports (lib/edid.js supports()) and the configs in play (the draft, what's
// saved, what's on screen). A control is hidden only when the panel rules it
// out and none of those uses it, so a setting that's there can still be undone.
function colorOffers(support, configs) {
  var uses = function (test) { return configs.some(function (c) { return c && test(c); }); };
  return {
    hdr: support.hdr !== false || uses(function (c) { return M.isHdr(c.cm) || (c.supports_hdr || 0) !== 0; }),
    tenBit: support.tenBit !== false || uses(function (c) { return c.bitdepth === 10; })
  };
}

// With both forced on, Hyprland's supportsHDR() is true whatever the EDID
// says, so a monitor still showing sRGB isn't a support decision: the rule
// hasn't landed yet, or something else is in the way.
function hdrForced(cfg) {
  return cfg.supports_hdr === 1 && cfg.supports_wide_color === 1;
}
